%% Cargar datos
Centro = load(['CasoIII_Resultados/Centro/Centro50'],'Referencia').Referencia;
Kobe = load(['CasoIII_Resultados/Kobe/Kobe50'],'Referencia').Referencia;
Maule = load(['CasoIII_Resultados/Maule/Maule50'],'Referencia').Referencia;
...
% exportgraphics(gcf,'Figs/CasoIII/Corte_casoIII.jpg',"Resolution",1000)
...
lh = legend('Orientation','horizontal');
lh.Layout.Tile = 'South';
% exportgraphics(gcf,['Figs/CasoIII/RespuestaMAX_casoIII.jpg'],"Resolution",1000)
...
lh = legend('Orientation','horizontal');
lh.Layout.Tile = 'South';
% exportgraphics(gcf,['Figs/CasoIII/RespuestaRMS_casoIII.jpg'],"Resolution",1000)
...
Referencia = load(['CasoIII_Resultados/Centro/Centro50'],'Referencia').Referencia;
...
ylabel('Fuerza [kN]');
xlim([10 30]);
% exportgraphics(gcf,['Figs/CasoIII/Respuesta_casoIII_FvsFA.jpg'],"Resolution",1000)
...
gcf = figure('Position', [10 10 1000 600]);
tiledlayout(3,1);
hf = nexttile;
...
grid on;
xlabel('Tiempo [s]');
title('Desplazamiento relativo [mm]');
xlim([0 Referencia.t(end)]);
...
hf = nexttile;
plot(t,d3sc.*1000,'k','DisplayName','Sin control');
hold on;
plot(t,d3cc.*1000,'--r','DisplayName','Control semi-activo');
grid on;
xlabel('Tiempo [s]');
title('Desplazamiento entrepiso [mm]');
xlim([0 Referencia.t(end)]);
text(hf.XLim(end)*0.7,hf.YLim(end)*0.85,['Reducción del maximo de ',num2str(d3max,2),' %'],'FontSize',8)
text(hf.XLim(end)*0.7,hf.YLim(end)*0.65,['Reducción del RMS de ',num2str(d3rms,2),' %'],'FontSize',8)
...
grid on;
xlabel('Tiempo [s]');
title('Aceleración absoluta [mm/s^2]');
xlim([0 Referencia.t(end)]);
...
% exportgraphics(gcf,['Figs/CasoIII/Respuesta_casoIII_RscvsRcc.jpg'],"Resolution",1000)

%% Cargar datos Centro diferentes magnitudes
% Escala 10%
Referencia = load(['CasoIII_Resultados/Centro/Centro10'],'Referencia').Referencia;
...
% Escala 30%
Referencia = load(['CasoIII_Resultados/Centro/Centro30'],'Referencia').Referencia;
...
% Escala 60%
Referencia = load(['CasoIII_Resultados/Centro/Centro60'],'Referencia').Referencia;
...
% Escala 10%
Referencia = load(['CasoIII_Resultados/Kobe/Kobe10'],'Referencia').Referencia;
...
% Escala 30%
Referencia = load(['CasoIII_Resultados/Kobe/Kobe30'],'Referencia').Referencia;
...
% Escala 60%
Referencia = load(['CasoIII_Resultados/Kobe/Kobe60'],'Referencia').Referencia;
...
% Escala 10%
Referencia = load(['CasoIII_Resultados/Maule/Maule10'],'Referencia').Referencia;
...
% Escala 30%
Referencia = load(['CasoIII_Resultados/Maule/Maule30'],'Referencia').Referencia;
...
% Escala 60%
Referencia = load(['CasoIII_Resultados/Maule/Maule60'],'Referencia').Referencia;
